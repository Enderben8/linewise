...
| 2026-09-22 | One page at a time holds the web database. `client.web.ts` waits up to 3 s for the `linewise-db` Web Lock, and a page whose open is still refused (the page it replaced has not let go of the files) reloads itself once. After that the error screen says Linewise is open in another tab and offers a reload | The browser's private file system allows one writer, and expo-sqlite's worker cannot open the database again after its first try fails |
